...
`Spring IoC(Inversion of Control) container`는 Spring Framework의 핵심 부분으로, 애플리케이션 객체의 생성, 구성, 생명주기 관리 등을 담당한다. IoC container는 객체(Bean)들의 의존 관계를 중앙에서 관리하며, 필요한 객체를 필요한 시점에 애플리케이션에 제공한다. 이를 통해 객체 간의 결합도를 중리고, 코드 재사용성, 유지보수성, 테스트 용이성을 향상시킨다.
